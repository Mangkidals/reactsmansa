...
const GUIDE_STEPS = [
  ...
  },
  {
    ...
    tips: [
      ...
      "⚠️ Level harus diselesaikan secara berurutan untuk membuka level berikutnya"
    ]
  },
  {
    icon: (
    ...
    title: "Langkah 3: Terapkan & Laporkan! 🛡️",
        ...
        <div className="px-6 sm:px-8 pb-6 sm:pb-8 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            {!isFirstStep && (
              <button
                onClick={handlePrev}
                className="px-4 py-2.5 text-xs font-bold text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-full transition-all cursor-pointer"
              >
                ← Kembali
              </button>
            )}
            <button
              onClick={handleNext}
              className="px-6 py-2.5 text-xs font-bold text-white rounded-full shadow-md transition-all cursor-pointer active:scale-95 hover:shadow-lg"
              style={{
                backgroundColor: step.color,
                boxShadow: `0 4px 14px ${step.color}40`
              }}
            >
              {isLastStep ? 'Mulai Jelajahi! 🚀' : 'Selanjutnya →'}
            </button>
          </div>
        </div>
